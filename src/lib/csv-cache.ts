...
class CsvCache {
  ...
  private loadCsvFile(csvPath: string): { set: Set<string>; map: Map<string, CsvData> } {
    try {
      ...
      const columns = this.splitCsvLine(header).map(c => c.trim().toLowerCase())
  ...
  private getFilePath(source: 'accounting' | 'consulting'): string {
    const filename = source === 'accounting' ? 'contaData.csv' : 'konsulentData.csv'
  ...
  getCsvData(source: 'accounting' | 'consulting'): { set: Set<string>; map: Map<string, CsvData> } {
